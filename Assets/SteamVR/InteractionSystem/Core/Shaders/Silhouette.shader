...
Shader "Valve/VR/Silhouette"
{
	...
	CGINCLUDE
		...
		PS_INPUT Extrude( PS_INPUT vertex )
		{
			...
			// Calculate position
#if UNITY_VERSION >= 540
			extruded.vPositionPs = UnityObjectToClipPos( vertex.vPositionOs.xyzw ) * length(ObjSpaceViewDir(vertex.vPositionOs.xyzw));
#else
			extruded.vPositionPs = mul( UNITY_MATRIX_MVP, vertex.vPositionOs.xyzw ) * length(ObjSpaceViewDir(vertex.vPositionOs.xyzw));
#endif
